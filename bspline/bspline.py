...
class Bspline:
    ...
    def __init__(self, knot_vector, degree):
        ...
        if kv.ndim > 1:
            raise ValueError(
                "knot_vector must be Python list or rank-1 array, but got rank = %d"
                % (kv.ndim)
            )
        self.knot_vector = kv
    ...
    def __basis0(self, xi):
        ...
        return np.where(
            np.all([self.knot_vector[:-1] <= xi, xi < self.knot_vector[1:]], axis=0),
            1.0,
            0.0,
        )
    ...
    def __basis(self, xi, p, compute_derivatives=False):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            first_term = np.where(
                ...
                (first_term_numerator / first_term_denominator),
                0.0,
            )
        ...
        return (
            first_term[:-1] * basis_p_minus_1[:-1] + second_term * basis_p_minus_1[1:]
        )

    def __cum_basis(self, xi, p, compute_derivatives=False):
        b = self.__basis(xi, p, compute_derivatives)
    ...
    @memoize
    def cum_basis(self, xi):
        """Convenience function to make the object callable.  Also 'memoized' for speed."""
        return self.__cum_basis(xi, self.p, compute_derivatives=False)
    ...
    def plot(self, x, N, N_cum):
        ...
        ax2.set_ylabel("cumulative basis")
        for n in N:
            ax1.plot(x, n)
        for n in N_cum:
            ax2.plot(x, n)
    ...
    def __diff_internal(self):
        ...
        """
        assert self.p > 0, (
            "order of Bspline must be > 0"
        )  # we already handle the other case in diff()
    ...
    def diff(self, order=1):
        ...
        if (
            order > self.p
        ):  # identically zero, but force the same output format as in the general case
            dummy = self.__call__(0.0)  # get number of basis functions and output dtype
            nbasis = dummy.shape[0]
            return lambda x: np.zeros(
                (nbasis,), dtype=dummy.dtype
            )  # accept but ignore input x
        ...
        for k in range(order):
            ...
            for Ci, Bi in terms:
                tmp.extend(
                    (Ci * cn, Bn) for cn, Bn in Bi.__diff_internal()
                )  # NOTE: also propagate Ci
            terms = tmp
